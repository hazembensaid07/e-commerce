...
//   }
  exports.signup = (req, res) => {
    const { username,email,first_name,last_name,birth_date,phone,location,profile_description, password } = req.body;
    ...
    User.findOne({ email }).exec((err, user) => {
      ...
      const token = jwt.sign(
        { username,email,first_name,last_name,birth_date,phone,location,profile_description, password, imgSrc },
  ...
  exports.accountActivation = (req,res) =>{
    ...
    if (token) {
      jwt.verify(token,process.env.JWT_ACCOUNT_ACTIVATION, function(err,decoded){
          ...
          const {username,email,first_name,last_name,birth_date,phone,location,profile_description, password, imgSrc} =jwt.decode(token)
          const user = new User({imgSrc,username,email,first_name,last_name,birth_date,phone,location,profile_description, password})
          user.save((err,user)=>{
            if(err){
  ...
  exports.signin = (req,res) => {
    ...
    User.findOne({email}).exec((err,user) =>{
      ...
      const { _id,username,email,first_name,last_name,birth_date,phone,location,profile_description,imgSrc} = user
      return res.json({
        message:'Signin succss.',
        token,
        user:{_id,username,email,first_name,last_name,birth_date,phone,location,profile_description,imgSrc},
      })
    })
  ...
  client.verifyIdToken({idToken, audience: process.env.GOOGLE_CLIENT_ID}).then(response=>{
    ...
    if(email_verified){
      User.findOne({email}).exec((err,user)=>{
        if(user){
          const token = jwt.sign({_id:user._id}, process.env.JWT_SECRET,{expiresIn:'7d'})
          const {_id,username,email,first_name,last_name,birth_date,phone,location,profile_description}=user
          return res.json({
            token, user : {_id,username,email,first_name,last_name,birth_date,phone,location,profile_description}
          })
        } else{
          ...
          user.save((err,data)=>{
            ...
            const {_id,username,email,first_name,last_name,birth_date,phone,location,profile_description}=data
            return res.json({
              token, user : {_id,username,email,first_name,last_name,birth_date,phone,location,profile_description}
            })
          })
